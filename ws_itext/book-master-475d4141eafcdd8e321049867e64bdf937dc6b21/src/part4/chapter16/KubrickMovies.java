...
public class KubrickMovies {
	...
	/**
	 * Creates a Collection schema that can be used to organize the movies of Stanley Kubrick in a collection: year, title, duration, DVD acquisition, filesize (filename is present, but hidden).
	 * 
	...
	public byte[] createPdf() throws DocumentException, IOException, SQLException {
		...
		for (Movie movie : movies) {
			fs = PdfFileSpecification.fileEmbedded(writer, null, String.format("kubrick_%s.pdf", movie.getImdb()), createMoviePage(movie));
	...
	public byte[] createMoviePage(Movie movie) throws DocumentException, IOException {
		...
		document.open();
		// step 4
		Paragraph p = new Paragraph(movie.getMovieTitle(), FontFactory.getFont(BaseFont.HELVETICA, BaseFont.WINANSI, BaseFont.NOT_EMBEDDED, 16));
		document.add(p);
