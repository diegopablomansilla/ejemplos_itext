...
public class DirectorPhrases1 {
	...
	/**
	 * Creates a Phrase with the name and given name of a director using
	 * different fonts.
	 * 
	 * @param rs
	 *            the ResultSet containing director records.
	 */
	public Phrase createDirectorPhrase(ResultSet rs)
			throws UnsupportedEncodingException, SQLException {
		Phrase director = new Phrase();
		director.add(new Chunk(new String(rs.getString("name").getBytes(), "UTF-8"),
				BOLD_UNDERLINED));
		director.add(new Chunk(",", BOLD_UNDERLINED));
		director.add(new Chunk(" ", NORMAL));
		director.add(new Chunk(new String(rs.getString("given_name").getBytes(), "UTF-8"),
				NORMAL));
		return director;
	}
	 ...
	 * 
	 * @param filename
	 *            the name of the PDF file that needs to be created.
	...
	public void createPdf(String filename) throws IOException,
		...
		ResultSet rs = stm
				.executeQuery("SELECT name, given_name FROM film_director ORDER BY name, given_name");
