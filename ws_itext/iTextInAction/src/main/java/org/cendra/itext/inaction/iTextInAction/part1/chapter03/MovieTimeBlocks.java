...
public class MovieTimeBlocks extends MovieTimeTable {
	...
	/**
	 * Draws a colored block on the time table, corresponding with the screening of a specific movie.
	 * 
	 * @param screening
	...
	protected void drawBlock(Screening screening, PdfContentByte under, PdfContentByte over) {
		under.saveState();
		BaseColor color = WebColors.getRGBColor("#" + screening.getMovie().getEntry().getCategory().getColor());
		under.setColorFill(color);
		Rectangle rect = getPosition(screening);
		under.rectangle(rect.getLeft(), rect.getBottom(), rect.getWidth(), rect.getHeight());
		under.fill();
		over.rectangle(rect.getLeft(), rect.getBottom(), rect.getWidth(), rect.getHeight());
		over.stroke();
		under.restoreState();
	}
